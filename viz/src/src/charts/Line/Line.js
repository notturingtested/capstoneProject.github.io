// Keep the commented out imports below...once we figure out how to use adobe's
// import process, we will change the imports to the correct form.
...
LineExpressionFunctions.registerStaticExpressions(vega);
/**
 * Parent class:
 ...
 */
export default class Line extends VegaViz {
	async draw(opts) {
	...
	get padding() {
		if (this.opts.shouldRenderSmallChart) {
			return { left: 2, top: 10, right: 2, bottom: 10 };
		} else {
			return { left: 10, top: 10, right: 10, bottom: 15 };
		}
	}
...
ChartFactory.registerChart(Line);
